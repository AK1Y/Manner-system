	...
	<div class="fater-body-show">
		<el-card shadow="never">
			...
			<div>
				<el-form :inline="true" :model="qryForm">
					<el-form-item>
						<el-input v-model="qryForm.name" placeholder="输入社团名称…" autocomplete="off"></el-input>
					</el-form-item>
					<el-form-item>
						<el-select v-model="qryForm.typeId" placeholder="请选择社团类型">
							<el-option label="查看全部" value=""></el-option>
							<el-option v-for="(item, index) in teamTypes" :key="index" :label="item.name"
		...
		<el-card shadow="never">
			<div v-if="userType == 0" slot="header">
				<el-button type="primary" style="font-size: 18px" @click="showAddWin()">
			...
			<div>
				<el-table v-loading="loading" element-loading-text="拼命加载中" element-loading-spinner="el-icon-loading"
					...
					<el-table-column v-if="userType == 0" align="center" width="250" label="操作处理" fixed="right">
						<template slot-scope="scope">
							<el-button type="primary" style="font-size: 18px" @click="showUpdWin(scope.row)">
								编辑</el-button>
							<el-button type="danger" style="font-size: 18px" @click="delInfo(scope.row.id)">
								删除</el-button>
						</template>
					</el-table-column>
					<el-table-column v-if="userType == 2" align="center" label="操作处理" fixed="right" width="140">
						<template slot-scope="scope">
							<el-button type="primary" style="font-size: 18px" @click="apply(scope.row.id)">
								申请</el-button>
						</template>
					</el-table-column>
				</el-table>
			...
			<!-- 确保图表容器的父级元素应用Flex布局并设置居中属性 -->
			
			<div style="display: flex; justify-content: center; align-items: center;">
			
			  <!-- 饼状图容器 -->
			
			  <div id="team-members-pie-chart" style="width: 700px; height: 400px;"></div>
		...
		<el-dialog title="添加信息" width="600px" :visible.sync="showAddFlag">
			<el-form label-width="90px" :model="teamsForm">
				...
				<el-form-item label="社团团长ID">
					<el-input v-model="teamsForm.manager" placeholder="请输入社团管理员ID…" autocomplete="off"></el-input>
				</el-form-item>
			</el-form>
...
<script>
	...
	export default {
		...
		methods: {
			...
			initTeamMembersPieChart() {
				const pieChartData = this.pageInfos.map(team => ({
				...
				const chartContainer = document.getElementById('team-members-pie-chart');
				...
				// 填充图表数据
				this.pageInfos.forEach((team) => {
					option.legend.data.push(team.name);
